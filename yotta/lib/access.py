...
def satisfyVersionFromAvailble(name, version_required, available):
    ...
    if name in available and available[name]:
        ...
        if spec and not spec.match(r.getVersion()):
            raise access_common.Unavailable('Previously added component %s@%s doesn\'t meet spec %s' % (name, r.getVersion(), spec))
        if name != r.getName():
            raise access_common.Unavailable('Component %s was installed as different name %s in %s' % (
