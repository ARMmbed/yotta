...
set(CMAKE_TOOLCHAIN_FILE "${replaceBackslash($toolchain_file)}")
...
add_subdirectory(
    "${replaceBackslash($srcdir)}"
    "${replaceBackslash($workingdir)}"
)
#end for
...
set(YOTTA_AUTO_${object_name.upper}_RESOURCE_FILES
    #echo '    ' + '\\n    '.join('"'+$replaceBackslash(x)+'"' for x in $resource_files) + '\\n'
)
#end if
...
set_target_properties($object_name PROPERTIES
    MACOSX_BUNDLE_INFO_PLIST ${replaceBackslash($file_name)}
)
#end if
#end for
...
add_executable($object_name
    #echo '    ' + '\\n    '.join('"'+$replaceBackslash(x)+'"' for x in $file_names) + '\\n'
)
#if 'objc' in $languages
\# no proper CMake support for objective-c flags :(
set_target_properties($object_name PROPERTIES
...
add_library($libname ${replaceBackslash($cfile_name)})
    ...
    def generate(self, builddir, component, active_dependencies, immediate_dependencies, all_dependencies, toplevel):
        ...
        for name, c in active_dependencies.items():
            add_depend_subdirs += string.Template(
                ...
                subdir=replaceBackslash(os.path.join(builddir, 'ym', name))
            )
